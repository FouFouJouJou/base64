...
* Building
The library is built both statically and dynamically. All one needs to do is run a couple =meson= commands. The =bin= directory is where I personally store all my builds since it suits [[https://en.wikipedia.org/wiki/Filesystem_Hierarchy_Standard][The Filesystem Hierarchy Standard]].
